...
impl Parser {
    pub fn parse_tokens<'a>(&self, tokens: &[Token]) -> LoxResult<Expr> {
        expression(tokens).map(|(expr, _)| expr)
    }
}

fn expression<'a>(tokens: &[Token]) -> LoxResult<(Expr, &[Token])> {
    equality(tokens)
}
...
fn unary<'a>(tokens: &[Token]) -> LoxResult<(Expr, &[Token])> {
...
fn primary(tokens: &[Token]) -> LoxResult<(Expr, &[Token])> {
    let token = tokens.first().expect("First token didn't exist");
    match &token.token_type {
        TokenType::Identifier(str_val) => Ok((Expr::Literal(LoxType::String_(str_val.clone())), &tokens[1..])),
        TokenType::Number(num) => Ok((Expr::Literal(LoxType::Number(*num)), &tokens[1..])),
        TokenType::String_(str_val) => Ok((Expr::Literal(LoxType::String_(str_val.clone())), &tokens[1..])),
